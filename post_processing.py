...
def filter_bilateral(data, min_length):
    ...
    for i in range(len(data)):
        ...
        if (i_r - i_l < min_length):
            new_y = data[i_l]
            new_data[i_l:i_r] = [new_y for _ in range(i_r-i_l)]
...
if __name__== "__main__":
    ...
    y_pred = np.load("Prediction/y_pred_with_val_nomfcc.npy")
    y_true = np.load("Prediction/y_true_with_val_nomfcc.npy")
    print ("Initial Accuracy: ", str(calculate_accuracy(y_true, y_pred)))
    #out = filter_by_min_length(test_y, 3)
   # for i in range(5):
    #   y_pred_smooth = filter_bilateral(y_pred_smooth, 10)#filter_by_min_length(y_pred, 5)#remove_single_entries(y_pred)
    y_pred_cont = butter_lpf(y_pred, .25, 10)
    #y_pred_smooth = filter_bilateral(y_pred, 10)
    y_pred_smooth = np.rint(y_pred_cont)
    #display_results_2(y_true,"Truth Data", y_pred, "Predicted")
    #display_results(y_true,"Truth Data", y_pred, "Raw Data", y_pred_smooth, "Smoothed Data")
    display_results(y_true,"Truth Data", y_pred_cont, "Continuous", y_pred_smooth, "Smoothed Data")
